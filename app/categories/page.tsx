...
export default function Categories(){
  return <main className="section"><div className="container">
    ...
    <div className="grid" style={{marginTop:28}}>{categories.map(([name,desc,icon])=><Link className="card category-card" href={`/talent?category=${encodeURIComponent(name)}`} key={name}>
      <div style={{fontSize:34,marginBottom:12}}>{icon}</div><h3>{name}</h3><p className="muted">{desc}</p><span className="btn">Explore →</span>
    </Link>)}</div>
  </div></main>
}
